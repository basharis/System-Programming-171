...
public abstract class Task<R> {
	...
	private Deferred<R> deferredResult = new Deferred<R>();;
	private Processor myProcessor;
	private AtomicInteger numTimesStarted = new AtomicInteger(0);
	private AtomicInteger numSubTasksToResolve;
	...
	//	System.out.println(Thread.currentThread().getName()+" is handling a task.");
		myProcessor = handler;
		if (numTimesStarted.getAndIncrement() == 0){
			start();
		}
		else{
			savedCallback.run();
	...
	protected final void whenResolved(Collection<? extends Task<?>> tasks, Runnable callback) {
		numSubTasksToResolve = new AtomicInteger(tasks.size());
		savedCallback = callback;
		for (Task<?> iter : tasks){	
			iter.deferredResult.whenResolved(()->{
				if (numSubTasksToResolve.decrementAndGet() == 0)
					spawn(this);});
		}
	}
