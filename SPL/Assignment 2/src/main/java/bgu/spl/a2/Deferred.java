...
public class Deferred<T> {
	...
	private ConcurrentLinkedQueue<Runnable> callbacksQueue;  // A queue designed for concurrent programs, holds the subscribed callbacks.
	...
	public T get() {
		if (isResolved())
			...
			throw new IllegalStateException("The deferred object you tried to get is not yet resolved");
	}
	...
	public synchronized void resolve(T value) {
		if (!resolved){
			deferredValue = value;
			resolved = true;
			/* Run the subscribed callbacks */
			while (!callbacksQueue.isEmpty()){
				callbacksQueue.remove().run();
			}
		}
		else{
			throw new IllegalStateException("This deferred has already been resolved");
		}
	}
	...
	protected int getNumberOfQueuedCallbacks() {
